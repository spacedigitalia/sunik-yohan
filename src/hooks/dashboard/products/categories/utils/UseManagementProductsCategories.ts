...
import { ProductsCategoriesPropes } from "../types/categories";

const ITEMS_PER_PAGE = 10;

export const useManagementProductsCategories = () => {
  const [items, setItems] = useState<ProductsCategoriesPropes[]>([]);
    ...
    try {
      ...
      if (page === 1) {
        q = query(
          collection(
            db,
            process.env.NEXT_PUBLIC_COLLECTIONS_PRODUCTS_CATEGORIES as string
          ),
